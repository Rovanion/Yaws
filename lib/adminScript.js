$(document).ready(function(){
	...
	$(Fname + "Container").find(Fname).append(
	...
	if(type == 0 || type == 2 || type == 4){
	    ...
	    $.post('../lib/operations.php', 
		   ...
		   function(data){
		       if(data.length > 4)
			   $(Tname).before('<div class="warning">' + data + '</div>');
		       else{
	...
	console.debug(Fname);
	if(type == 'Form'){
	    formData = $(Fname + 'Container').find(Fname).serializeArray();
	    textAreaData = $(Fname + 'Container').find('.nicEdit-main').html();
	    formData.push({ name: 'content', value: textAreaData});
	    formData.push({name: "title", value: $(Fname + ' [name|="title"]').val()});
	    
	    //Empties the fields for future use
	    $(Fname + 'Container').find('.nicEdit-main').html('<p></p>');
	    $(Fname + ' [name|="title"]').val('');
	}
	else{
	    formData = $(name + "SaveForm").serializeArray();
	    formData.push({ name: 'content', value:  $(name + 'Text').html()});	    
...
function fetchSingleEntry(formData, data, formName){
    var postData = { onePart: 'true', index: data, 
		     page: formData[3].value };
    
    $.post('../lib/content.php', postData,
	   function(data){
	       /* First attatch the data fetched from content.php. */
	       $(formName).parent().prev().prev().children().last().after(data);
				  
	       //Then do some operations on it
	       name = $(formName).parent().prev().prev().children().last().attr('id');	
	       name = name.substring(0, (name.length - 15));
