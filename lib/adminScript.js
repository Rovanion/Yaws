$(document).ready(function(){
    ...
    $('#PageUnsortedList').delegate('.cancel', 'click', function(){
    ...
    $('#PageUnsortedList').delegate('.submit', 'click', function(){
	var Cname = '#' + $(this).parent().attr('id');
	...
	$.post('../lib/operations.php', formData,
	       function(data){
		   if(data.length > 16)
		       alert(data);  //Display error if result is long enough to be one
		   else if(type == 'Form'){
		       var postData = { onePart: 'true', index: data, 
					page: formData['table'] };
		       $.post('../lib/content.php', postData,
			      function(data){ 
				  $(ULname).find('li').last().after(data);
				  
				  /* The buttons must be hid and the events must be bound.
				     Height of categories and hidden objects must be put
				     into originalheight. */
			      });
		   }
	       });
